...
public class HeraldyTextures
{
    public static final ResourceLocation BANNER_BASE_TEXTURE = new ResourceLocation(KnightlyArmory.ID, "textures/entity/banner/base.png");

    @SideOnly(Side.CLIENT)
    public static class Cache
        {
            private final Map<String, HeraldyTextures.CacheEntry> cacheMap = Maps.<String, HeraldyTextures.CacheEntry>newLinkedHashMap();
            ...
            public ResourceLocation getResourceLocation(String id, List<BannerPattern> patternList, List<EnumDyeColor> colorList)
            {
                ...
                {
                    id = this.cacheId + id;
                    HeraldyTextures.CacheEntry bannertextures$cacheentry = this.cacheMap.get(id);

                    if (bannertextures$cacheentry == null)
                    {
                        if (this.cacheMap.size() >= 256 && !this.freeCacheSlot())
                        {
                            return HeraldyTextures.BANNER_BASE_TEXTURE;
                        }
                        ...
                        bannertextures$cacheentry = new HeraldyTextures.CacheEntry();
            ...
            private boolean freeCacheSlot()
            {
                ...
                while (iterator.hasNext())
                {
                    ...
                    HeraldyTextures.CacheEntry bannertextures$cacheentry = this.cacheMap.get(s);
